...
/*RisParser|BibParser to JSON | Vice Versa | Colwiz.com*/
function RisParser(arg0, arg1) {
    if(typeof arg1 == "undefined"){
        arg1 = {};
    }
    var risFormat = {
        "AU": "author","A1": "authors","AB": "abstract","DO": "EntryKey","DA": "date","ET": "issue","ET2": "edition",
        "EP": "endpage","ER": "eof","JO": "journal","KW": "keywords","LA": "language","PB": "publisher",
        "PY": "year","SN": "isbn","SN2": "issn","SP": "startpage","TY": "EntryType","TI": "title","UR": "url",
        "VL": "volume","Y1": "year","AD": "otherinfo","AN": "accessionNumber","CA": "caption","CN": "number",
        "CY": "publisher","DP": "database","IS": "number","J2": "title","L1": "fileattachment",
        "L4": "figure","LB": "label","M1": "number","M3": "type","N1": "notes","NV": "numberofvolumes",
        "OP": "originalpublication","RI": "revieweditem","RN": "researchnotes","RP": "reprintedition","SE": "section",
        "ST": "title","T1": "title","T2": "title","T3": "title","TA": "title", "TT": "title",
        "Y2": "accessdate", "JF": "journal", "JA": "journal", "L3": "fileattachments"
    };
    var types = {
        "JOUR": "article", "ABST": "abstract", "ADVS": "audiovisualmaterial", "AGGR": "aggregateddatabase", "ANCIENT": "ancienttext", "ART": "artwork",
        "BILL": "bill", "BLOG": "blog", "BOOK": "inproceedings", "CASE": "case", "CHAP": "bookchapter", "CHART": "chart", "CLSWK": "classicalwork",
        "COMP": "computerprogram", "CONF": "conferenceproceeding", "CPAPER": "conferencepaper", "CTLG": "catalog", "DATA": "datafile", "DBASE": "onlinedatabase",
        "DICT": "dictionary", "EBOOK": "electronicbook", "ECHAP": "electronicbooksection", "EDBOOK": "editedbook", "EJOUR": "electronicarticle",
        "ELEC": "webpage", "ENCYC": "encyclopedia", "EQUA": "equation", "FIGURE": "figure", "GEN": "generic", "GOVDOC": "governmentdocument",
        "GRANT": "grant", "HEAR": "hearing", "ICOMM": "internetcommunication", "INPR": "inpress", "JFULL": "journal(full)","LEGAL": "legalruleorregulation",
        "MANSCPT": "manuscript", "MAP": "map", "MGZN": "magazinearticle", "MPCT": "motionpicture", "MULTI": "onlinemultimedia", "MUSIC": "musicscore",
        "NEWS": "newspaper", "PAMP": "pamphlet", "PAT": "patent", "PCOMM": "personalcommunication", "RPRT": "report", "SER": "serialpublication",
        "SLIDE": "slide", "SOUND": "soundrecording", "STAND": "standard", "STAT": "statute", "THES": "thesis/dissertation", "UNPB": "unpublishedwork",
        "VIDEO": "videorecording"
    };
    var ty = arg0.indexOf("TY  -"),
        er = arg0.lastIndexOf("ER  -");
    var rises = arg0.slice(ty, er), risBlock = [], multipleRis = [];
    rises = rises.split("ER  -");
    CwZ.each(rises, function(i){
        risBlock.push(rises[i].split("TY  -")[1]);
    });
    CwZ.each(risBlock, function(i){
        var risBlocks = risBlock[i];
        if(risBlocks.length && !ccUtils.checkHtmlTags.test(risBlocks))
            multipleRis.push(risBlocks);
    });
    rises = multipleRis.join("ER  -\n\nTY  -");
    rises = "TY  -" + rises;
    rises = CwZ.trim(rises).split("ER  -");
    //rises.splice(rises.length-1,1);
    var risCollector = {
        entries: []
    };
    var isEmptyPub = CwZ.isEmptyObject(arg1);
    var isMultipleAuthors = false;
    for(var RIS in rises){
        if(!!~rises[RIS].indexOf("TY  -")) {
            var ris = CwZ.trim(rises[RIS]);
            var risObj = {};
            ris = ris.split('\n');
            var risFields = {};

            for (var field in ris) {
                var val = CwZ.trim(ris[field]);
                var indexOfSplitter = val.indexOf('-');
                if (!!~indexOfSplitter) {
                    field = ccUtils.splitValue(val, indexOfSplitter);
                    var key = CwZ.trim(field[0]), value = CwZ.trim(field[1]);
                    if (key.length == 2) {
                        var fieldLeaf = risFields[key];
                        if (!fieldLeaf) {
                            risFields[key] = value;
                        } else {
                            var isAuthorField = key;
                            if (isAuthorField[0] == "A" && (isAuthorField[1] == "U" || !isNaN(parseInt(isAuthorField[1])))) {
                                risFields[key] = fieldLeaf + " and " + value;
                            } else {
                                risFields[key] = value;
                            }
                        }
            ...
            var authors = risFields["AU"]? risFields["AU"]: "";
            var idx = 1;
            while (typeof risFields["A" + idx] != "undefined" || typeof risFields["A" + (++idx)] != "undefined") {
                isMultipleAuthors = true;
                authors += (authors != ""? " and " :"") + risFields["A" + idx];
                delete risFields["A" + idx];
            }
            if (authors == "") {
                authors = ((!isEmptyPub && arg1.authors != "") ? arg1.authors : "");
            } else {
                delete risFields["AU"];
            }
            var fields = {};
            risObj[risFormat["DO"]] = risFields["DO"] || ((!isEmptyPub && arg1.identifier != "") ? arg1.identifier : "");
            delete risFields["DO"];
            risObj[risFormat["TY"]] = types[risFields["TY"]] || types["JOUR"];
            delete risFields["TY"];
            fields[risFormat["A" + (isMultipleAuthors ? "1" : "U")]] = authors;
            for (var objs in risFields) {
                if(!fields[risFormat[objs]]){
                    if (typeof risFormat[objs] != "undefined") {
                        fields[risFormat[objs]] = risFields[objs]
                    } else {
                        fields[objs] = risFields[objs]
                    }
                }
            }
            risObj["Fields"] = fields;
            delete risObj["Fields"]["eof"];
            risCollector.entries.push(risObj);
        }
    }
    return risCollector;
} // end function RisParser
...
// From Javascript Object to BiBTeX
function BibTeXGenerator(jsobj){
    var bibTexts = [];
    for(var i=0;i<jsobj.length;i++) {
        var entry = jsobj[i];
        var bibStart, bibEnd = "\n}";
        bibStart = "@" + entry.EntryType + "{" + entry.EntryKey + ",\n";
        ...
        str = str.slice(0, -2);
        bibTexts.push(bibStart + str + bibEnd);
    }
    return bibTexts;
}

// From Javascript Object to RIS
function RisGenerator(jsobj){
    var risTexts = [];
    var AllTypes = ["article", "inproceedings", "incollection", "book", "inbook", "abstract", "audiovisualmaterial", "aggregateddatabase", "ancienttext", "artwork",
        "bill", "blog", "wholebook", "case", "bookchapter", "chart", "classicalwork", "computerprogram", "conferenceproceeding", "conferencepaper",
        "catalog", "datafile", "onlinedatabase", "dictionary", "electronicbook", "electronicbooksection", "editedbook", "electronicarticle",
        "webpage", "encyclopedia", "equation", "figure", "generic", "governmentdocument", "grant", "hearing", "internetcommunication", "inpress",
        "journal(full)", "journal", "legalruleorregulation", "manuscript", "map", "magazinearticle", "motionpicture", "onlinemultimedia",
        "musicscore", "newspaper", "pamphlet", "patent", "personalcommunication", "report", "serialpublication", "slide", "soundrecording",
        "standard", "statute", "thesis/dissertation", "unpublishedwork", "videorecording"];
    var types = {
        "article":"JOUR", "abstract":"ABST", "audiovisualmaterial":"ADVS", "aggregateddatabase":"AGGR", "ancienttext":"ANCIENT",
        "artwork":"ART", "bill":"BILL", "blog":"BLOG", "wholebook":"BOOK", "case":"CASE", "bookchapter":"CHAP", "chart":"CHART",
        "classicalwork":"CLSWK", "computerprogram":"COMP", "conferenceproceeding":"CONF", "conferencepaper":"CPAPER", "catalog":"CTLG",
        "datafile":"DATA", "onlinedatabase":"DBASE", "dictionary":"DICT", "electronicbook":"EBOOK", "electronicbooksection":"ECHAP",
        "editedbook":"EDBOOK", "electronicarticle":"EJOUR", "webpage":"ELEC", "encyclopedia":"ENCYC", "equation":"EQUA", "figure":"FIGURE",
        "generic":"GEN", "governmentdocument":"GOVDOC", "grant":"GRANT", "hearing":"HEAR", "internetcommunication":"ICOMM", "inpress":"INPR",
        "journal(full)":"JFULL", "journal":"JOUR", "legalruleorregulation":"LEGAL", "manuscript":"MANSCPT", "map":"MAP", "magazinearticle":"MGZN",
        "motionpicture":"MPCT", "onlinemultimedia":"MULTI", "musicscore":"MUSIC", "newspaper":"NEWS", "pamphlet":"PAMP", "patent":"PAT",
        "personalcommunication":"PCOMM", "report":"RPRT", "serialpublication":"SER", "slide":"SLIDE", "soundrecording":"SOUND", "standard":"STAND",
        "statute":"STAT", "thesis/dissertation":"THES", "unpublishedwork":"UNPB", "videorecording":"VIDEO", "inproceedings": "BOOK", "incollection": "BOOK",
        "inbook": "BOOK", "book": "BOOK"
    };
    for(var idx=0; idx<jsobj.length; idx++){
        var entry = jsobj[idx];
        var risStringCollection = "";
        var type = "", authors = "", abstractText = "", title = "", journal = "", date = "",
            doi = "",edition = "", keywords = "", language = "", publisher = "",
            year = "", isbn = "", startPage = "", endPage = "", url = "", volume = "",
            endOfReference = "ER  - \n\n";
        var isKnownType = CwZ.inArray(entry.EntryType, AllTypes);
        if(entry.EntryKey)
            doi =  "DO  - " + entry.EntryKey + "\n";
        type = ("TY  - " + (types[AllTypes[isKnownType]] || types["article"]) + "\n");

        entry = entry.Fields || entry.fields;

        if(entry.author || entry.authors){
            var author = entry.author || entry.authors;
            author = author.split(" and ");
            authors = "";
            for(var i=0;i<author.length;i++){
                if(i==0) authors += "AU  - " + author[i];
                else authors += "A"+(i+1)+"  - " + author[i];
                authors += "\n"
            }
        }

        if(entry.abstract)
            abstractText = "AB  - " + entry.abstract + "\n";

        if(entry.title)
            title = "TI  - " + entry.title + "\n";

        if(entry.journal)
            journal = "JO  - " + entry.journal + "\n";
        if(entry.date || entry.year ){
            try{
                var start = "DA  - ", end = "\n", ndate = "", computedDate = "";
                if(entry.date) {
                    ndate = new Date(entry.date);
                    if(!isNaN(ndate)){
                        computedDate = ndate.getFullYear() + "/" + (ndate.getMonth() + 1) + "/" + ndate.getDate();
                    } else{
                    ...
                    if(entry.month){
                        ...
                        ndate = new Date(month + " " + year) || "";
                        ndate = !(isNaN(ndate)) ? ndate : "";
                        if(ndate){
                            computedDate = ndate.getFullYear() + "/" + (ndate.getMonth() + 1);
                        } else{
                            throw new Error("Invalid Date");
                        }
                    } else{
                        ...
                        ndate = !(isNaN(ndate)) ? ndate : "";
                        if(ndate){
                            computedDate = ndate.getFullYear();
                        } else{
                            throw new Error("Invalid Date");
                        }
                    }
                }
                if(computedDate.toString().toLowerCase() != "nan" || computedDate.toString() != ""){
                    var matches = computedDate.toString().match(/(\/)/g);
                    if (matches) {
                    ...
                    if(!isNaN(new Date(computedDate))){
                        date +=  start + computedDate + end;
                        year = "";
                    }
                }
            } catch(ex){
        ...
        if(entry.issue)        edition = "ET  - " + entry.issue + "\n";
        else if(entry.edition) edition = "ET  - " + entry.edition + "\n";

        if(entry.isbn)      isbn = "SN  - " + entry.isbn + "\n";
        else if(entry.issn) isbn = "SN  - " + entry.issn + "\n";

        if(entry.keywords)
            keywords = "KW  - " + entry.keywords;

        if(entry.language)
            language = "LA  - " + entry.language + "\n";

        if(entry.publisher)
            publisher = "PB  - " + entry.publisher + "\n";

        if(entry.startPage)
            startPage = "SP  - " + entry.startPage + "\n";

        if(entry.endPage)
            endPage = "EP  - " + entry.endPage;

        if(entry.url)
            url = "UR  - " + entry.url + "\n";

        if(entry.volume)
            volume = "VL  - " + entry.volume + "\n";

        risStringCollection += (type + authors + abstractText + title + journal + date + doi + edition + keywords + language + publisher + year + isbn + startPage + endPage + url + volume + endOfReference);
        risTexts.push(risStringCollection);
    }
    return risTexts;
}

function converter(jsObj, type, returnType){
    ...
    try{
        switch (type){
            case "bib":
                text = BibTeXGenerator(arr);
                break;
            case "ris":
                text = RisGenerator(arr);
                break;
            default:
...
function BibToRis(bibText){
    var jsObj = BibtexParser(bibText);
    return converter(jsObj.entries, "ris", "string");
}
function RisToBib(ris, optPub){
    var jsObj = RisParser(ris, optPub);
    return converter(jsObj.entries, "bib", "string");
}

function citConverterMain(content, conversionType, optPub){
    if(typeof optPub == "undefined"){
        optPub = {};
    }
    var returnVal = "";
    try{
        if(conversionType == "ris"){
            returnVal = BibToRis(content, optPub)
        } else if(conversionType == "bib"){
            returnVal = RisToBib(content, optPub)
        }
    } catch(ex){
    } finally{
        return returnVal;
...
function citJSONMain(content, contentType, optPub){
    if(typeof optPub == "undefined"){
        optPub = {};
    }
    var returnVal = "";
    try{
        if(contentType == "ris"){
            returnVal = RisParser(content);
        } else if(contentType == "bib"){
            returnVal = BibtexParser(content);
        }
    } catch(ex){
    } finally{
        return returnVal;
...
var ccUtils = {
    checkHtmlTags: new RegExp("<[^>]+>"),
    trim: function(str){
        if (typeof str == "string" && str && str.length)
            str = CwZ.trim(str).replace(/^\s+|\s+$|\s+(?=\s)/g, '');
        else
            str = "";
        return str;
    },
