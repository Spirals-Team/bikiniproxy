		...
		----------------------------*/
	 	_init : function(){
			...
			if( !jQuery("#bk-header").hasClass("bk-collapsed-header") ) {
				  $(vars.thumb_tray).css({bottom : -$(vars.thumb_tray).height(), opacity: '0'});
				  $(vars.next_slide).hide();
				  $(vars.prev_slide).hide();
				  $(vars.slide_caption).hide();
			} else {
				$("#bk-content-inner-wrap").hide();
			}		  
    		...
    		/*
    		 * Handle jQuery swipe plugin for touch devices
    		 */
    		if(jQuery().swipe) {
			...
			jQuery(window).on('bk_navigation_about_to_close', function() {
				  ...
				  if( api.options.slides.length > 1 ) {
				    $(vars.thumb_tray).animate({bottom : '0px', opacity: '1'}, 'slow');
				    $(vars.next_slide).fadeIn('slow');
				    $(vars.prev_slide).fadeIn('slow');
				  }
			...
			jQuery(window).on('bk_navigation_about_to_open', function() {
					$(vars.thumb_tray).animate({bottom : -$(vars.thumb_tray).height(), opacity: '0'}, 'slow');
					$(vars.next_slide).fadeOut('slow');
				    $(vars.prev_slide).fadeOut('slow');
				    $(vars.slide_caption).fadeOut('slow');
			});
