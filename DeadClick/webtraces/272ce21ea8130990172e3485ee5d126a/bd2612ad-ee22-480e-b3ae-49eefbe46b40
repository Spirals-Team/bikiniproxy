(function(name){
    ...
    var customizations = {};
    var sendEvent = function(tracking_command,event_name,params){
	      if(customizations.pcid){
	          params.product_catalog_id = customizations.pcid;
	      }
	      if(window.fbq){
	          fbq(tracking_command,event_name,params);
	      }	
    };
    var sendViewContentEvent = function(){
    ...
    var setLatestOldVisit = function(){
	      ...
	      if(localStorage){
	          if(oldest_visit_date){
                ...
                if(keywee_history[oldest_visit_date]){
		                localStorage.keywee_oldest_visit_data = JSON.stringify(keywee_history[oldest_visit_date]);
                }
	          }
	          else{
		            localStorage.removeItem("keywee_oldest_visit_date");
		            localStorage.removeItem("keywee_oldest_visit_data");
	          }
	      }
	      return oldest_visit_date;
    };
    ...
    var addContentPathToEvent = function(event){
        if(category){
            event["cat-" + category] = 1;
	          event.category = category;
        }
        if(section){
            event["sect-" + section] = 1;
	          event.section = section;
        }
        if(sub_section){
            event["sub-sect-" + sub_section] = 1;
	          event.sub_section = sub_section;
        }
        return event;
    };
    var getPeriodicEventToSend = function(){
        var event = {
	          tsop: last_time_spent_on_page,
	          lop: max_vertical_position
	      };
        return addContentPathToEvent(event);
    };
    var updateIsCoreAudience = function(event){
        if(this.customizations.isCore){
            this.customizations.isCore.bind(this)(event);
        }
        else{
	          if(event.lov && event.noad30 >= 2){
	              event.core = 1;
	          }
        }
    };
    var getEventToSend = function(){
	      ...
	      var event = {
	          ...
	          noadiccm: flat_current_category_this_month.length,
	          noadcm: num_of_active_days_cal_month
	      };
        event = addContentPathToEvent(event);
	      if(latest_old_visit){
	      ...
	      if(this.customEventData){
	          event = extend(event,this.customEventData);
	      }
        this.updateIsCoreAudience(event);
	      return event;
    };
    ...
    var sendPeriodicEvent = function(){
	      ...
	      if(postion_changed && ! too_much_tsop){
	          updateData();	
	          event = this.getPeriodicEventToSend();
	          sendEvent('trackCustom', 'KWCEPA',event);
	          previous_vertical_postion = current_vertical_location;
	      }
    };
    var sendPageViewEvent = function(){
	      var event = null;
	      updateData();
	      event = this.getEventToSend();
	      sendEvent('trackCustom', 'KWCEPV',event);
        if(window.fbq){
            fbq('track', "PageView");
        }
        if(window.snowplowKW){
    ...
    var init = function(options){
	      var that = this;        
        if(options.waitFor && !options.waitFor() && wait_retries > 0){
            wait_retries = wait_retries - 1;
            setTimeout(this.init,5000,options);
            return;
        }
	      try {
	          ...
	          if(customizations.getCategories){
		            this.categories = customizations.getCategories();
                if(this.categories.length > 0){
                    category = slugifyString(this.categories[0]);
                }
                if(this.categories.length > 1){
                    section = slugifyString(this.categories[1]);
                }
                if(this.categories.length > 2){
                    sub_section = slugifyString(this.categories[2]);
                }
	          }
	          if(customizations.eventListners){
		            customizations.eventListners.forEach(function(listner){
		                listner.bind(that)();
		            });
	          }
	          if(category){
                sendViewContentEvent();
		            ...
		            setInterval(sendPeriodicEvent.bind(this),30000);	
	          }
	      }
	      catch (e) {
    ...
    var Keywee = {};   
    window[name] = Keywee;
    Keywee.last_30_days_date_str = last_30_days_date_str;
    Keywee.getDataBetweensDates = getDataBetweensDates;
    Keywee.updateIsCoreAudience = updateIsCoreAudience;
    ...
    Keywee.getPeriodicEventToSend = getPeriodicEventToSend;
    Keywee.cleanOldData = cleanOldData;
    Keywee.sendPeriodicEvent = sendPeriodicEvent;
    Keywee.sendPageViewEvent = sendPageViewEvent;
    Keywee.sendViewContentEvent = sendViewContentEvent;
    Keywee.getLatestOldVisit = getLatestOldVisit;
    Keywee.setLatestOldVisit = setLatestOldVisit;
    Keywee.sendEvent = sendEvent;
    Keywee.product_id = product_id;
    Keywee.init = init;
    return Keywee;    
})("Keywee");
...
(function(){
	...
	var section = null;
	var section_element = document.querySelector('meta[name="SCG"]');
	if(section_element){
