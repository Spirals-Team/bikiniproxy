...
function casaleCall(CasaleID,Args){var divID='CasaleDIV'+Math.random();document.write('<div id="'+divID+'" style="width:0; height:0;position:absolute;"></div>');var casaleU='';var amp='&amp;';var casaleIF=0;var adUnitList=Args.adUnits;if(Args.version>=5){adUnitList=Args.defaultAdUnit;}
if(top==self){casaleU=encodeURIComponent(top.location.href);}else{casaleU=encodeURIComponent(document.referrer);casaleIF=1;}
var urlRegex=/^(?:[hH][tT][tT][pP][sS]?:\/\/)?((?:[a-zA-Z0-9]{1}(?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9]{1})?\.)*[a-zA-Z0-9]{1}(?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9]{1})?)(?:\:[0-9]{1,5})?(?:\/[^\?\#]*?(?:\?(?:[^\#]*?))?(?:\#.*)?)?$/;var usingReferrer=0;if(Args.referrer&&typeof Args.referrer==='string'&&urlRegex.test(Args.referrer)&&urlRegex.exec(Args.referrer)[1].length<=255){casaleU=encodeURIComponent(Args.referrer);usingReferrer=1;}
...
var req='<script type="text/javascript" src="'+casaleDomain+'/j?s='+CasaleID+amp+'a='+adUnitList+amp+'id='+CasaleR+amp+'p='+CasaleP+amp+'v='+Args.version+amp+'inif='+casaleIF;if(Args.positionID){req+=amp+'pos='+Args.positionID;}
